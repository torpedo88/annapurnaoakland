...
/**
 * The "Popular right now" feed: the real top-selling items (last 90 days, paid
 * orders), resolved against the live DB catalog. We surface FOUR DIVERSE dishes
 * — ranked by real sales, but preferring one per category and dishes that have
 * a real photo — so it reflects demand without showing four of the same thing.
 * Falls back to the catalog (still diverse) when there are no recent orders
 * (e.g. the isolated dev DB).
 *
 * Shared by the homepage server component (which renders these into the HTML so
 * the dish names are crawlable and the section reserves its own height) and by
 * `/api/menu/popular`.
 */
